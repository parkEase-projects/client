...
const Home = () => {
  ...
  const handleSearch = () => {
    if (selectedArea && selectedDate && selectedTime) {
      const selectedAreaObj = areas.find(a => a.id === parseInt(selectedArea));
        ...
        <Grid container spacing={3} alignItems="center">
          <Grid item xs={12} md={4}>
            <FormControl fullWidth size="small">
              <label style={{ display: 'block', marginBottom: 8 }}>Parking Area</label>
              {loading ? (
          ...
          <Grid item xs={12} md={4}>
            ...
            <input
              ...
              min={today}
              style={{ width: '100%', padding: 8, borderRadius: 4, border: '1px solid #ccc', cursor: 'pointer' }}
            />
          </Grid>
          ...
          <Grid item xs={2} md={1} sx={{ display: 'flex', alignItems: 'end', justifyContent: 'center' }}>
            <IconButton 
              color="primary" 
              onClick={handleSearch} 
              sx={{ mt: 3 }}
              disabled={loading || !selectedArea || !selectedDate || !selectedTime}
            >
              <SearchIcon />
            </IconButton>
          </Grid>
